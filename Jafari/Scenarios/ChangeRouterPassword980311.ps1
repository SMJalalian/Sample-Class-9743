...
$Cred = Get-Credential
$Command1 = "username admin privilege 15 secret "
$Output = @()


foreach ( $Record in $AllRecords ) {
    $Key = New-RandomPassword 10 -Lowercase -Uppercase -Numbers
    $Command = $Command1 + $Key   
    ...
    $Auth | Add-Member -MemberType NoteProperty -Name RouterName -Value $Name[0]
    $Auth | Add-Member -MemberType NoteProperty -Name RouterIP -Value $IP
    $Auth | Add-Member -MemberType NoteProperty -Name Password -Value $Key
    $Output += $Auth
    
    Get-SSHSession | Remove-SSHSession               
}

$Output | Export-Csv "C:\Projects\class-9743\Jafari\Scenarios\RouterUsers.csv" -Encoding UTF8