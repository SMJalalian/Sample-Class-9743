...
$AllRecords = Get-DnsServerResourceRecord -ZoneName PowerShell.Local -ComputerName DC | Where-Object -Property HostName -Like AS-*
$Cred = Get-Credential
...
foreach ( $Record in $AllRecords ) {
    ...
    $Key=New-RandomPassword -Length 10 -Uppercase -Lowercase -Numbers
    ...
    $x | Add-Member -MemberType NoteProperty -Name RoterKey -Value $Key
    $x | Add-Member -MemberType NoteProperty -Name RoterName -Value $Name
    $x | Add-Member -MemberType NoteProperty -Name RoterIP -Value $IP
    $ObjectArray += $x
    
    ######################################################################
    
    $SSH = New-SSHSession -ComputerName $IP -Credential $Cred -AcceptKey -ConnectionTimeout 30
    ...
    $Stream.WriteLine("username localadmin privilege 15 secret $Key")     
    Start-Sleep -Seconds 5
...
$ObjectArray | Export-Csv -Path "C:\Users\0946907374\Desktop\Router.csv" -Encoding UTF8
